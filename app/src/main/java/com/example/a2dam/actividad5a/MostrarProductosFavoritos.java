 ...
 */
public class MostrarProductosFavoritos extends Fragment {
    ...
    public MostrarProductosFavoritos() {
    ...
    public static MostrarProductosFavoritos newInstance(String param1, String param2) {
        MostrarProductosFavoritos fragment = new MostrarProductosFavoritos();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        DatabaseReference dbProductosFavoritos = FirebaseDatabase.getInstance().getReference("Favoritos");

        recyclerView = v.findViewById(R.id.listaProductos);
        recyclerView.setLayoutManager(new LinearLayoutManager(getContext(),LinearLayoutManager.VERTICAL,false));

        final ArrayList<String> favoritos = new ArrayList<>();
        dbProductosFavoritos.addListenerForSingleValueEvent(new ValueEventListener() {

            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                for (DataSnapshot datasnapshot: dataSnapshot.getChildren()) {
                    String favorito = datasnapshot.getValue(String.class);
                    if (favorito.contains(MainActivity.usuarioSesion.getUsuario())){
                        favoritos.add(favorito);
                    }
                    Toast.makeText(getContext(),favoritos.toString(),Toast.LENGTH_SHORT).show();
                }
